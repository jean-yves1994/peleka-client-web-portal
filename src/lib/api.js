...
export function saveAuth(data) {
  ...
  if (data?.access_token)
    localStorage.setItem("peleka_access_token", data.access_token);
  if (data?.refresh_token)
    localStorage.setItem("peleka_refresh_token", data.refresh_token);
}
...
export const api = {
  ...
  verifyPhoneReset: (phone, code) =>
    request("/auth/password-reset/phone/verify", {
      method: "POST",
  ...
  me: () => request("/me"),
  shipments: (params = "") =>
    request(`/shipments${params ? `?${params}` : ""}`),
  shipment: (id) => request(`/shipments/${id}`),
  quote: (body) =>
    request("/shipments/quote", { method: "POST", body: JSON.stringify(body) }),
  createShipment: (body) =>
    request("/shipments", { method: "POST", body: JSON.stringify(body) }),
  cancelShipment: (id, reason) =>
    request(`/shipments/${id}/cancel`, {
      method: "POST",
  ...
  rating: (id, score, comment) =>
    request(`/shipments/${id}/rating`, {
      method: "POST",
      body: JSON.stringify({ score, comment }),
    }),
  searchLocations: (q, lat, lng) =>
    request(
      `/locations/search?q=${encodeURIComponent(q)}${lat != null ? `&lat=${lat}` : ""}${lng != null ? `&lng=${lng}` : ""}`,
    ),
  reverseLocation: (lat, lng) =>
    request(`/locations/reverse?lat=${lat}&lng=${lng}`),
  initiatePayment: (shipment_id, phone) =>
    request("/payments/paypack/initiate", {
      method: "POST",
      body: JSON.stringify({ shipment_id, phone }),
    }),
  payment: (id) => request(`/payments/${id}`),
  billing: () => request("/me/billing"),
  profile: (body) =>
    request("/me", { method: "PATCH", body: JSON.stringify(body) }),
