...
async function request(path, options = {}) {
  ...
  if (!res.ok) {
    const isAuthEndpoint = path === "/auth/login" || path === "/auth/register" || path === "/auth/forgot-password" || path.startsWith("/auth/password-reset/") || path === "/auth/reset-password";
    if (res.status === 401 && !isAuthEndpoint) redirectToLoginOnExpiredSession(path);
...
export const api = {
  ...
  searchLocations: (q, lat, lng, limit = 8) =>
    request(`/locations/search?q=${encodeURIComponent(q)}${lat != null ? `&lat=${lat}` : ""}${lng != null ? `&lng=${lng}` : ""}&limit=${Math.min(10, Math.max(1, Number(limit) || 8))}`),
