...
export default function Shipments() {
  ...
  async function load() {
    ...
    try {
      const r = await api.shipments(
        `pageSize=100${status ? `&status=${status}` : ""}`,
      );
      setRows(r?.data || r?.items || []);
    } catch (e) {
    } finally {
      setLoading(false);
    }
  ...
  const filtered = rows.filter((s) =>
    `${s.tracking_number} ${s.pickup_address} ${s.delivery_address}`
      .toLowerCase()
  ...
  return (
    <div>
      <div className="page-title-row">
        <div>
          <div className="page-kicker">DELIVERIES</div>
          <h1>Your shipments</h1>
          <p>Every shipment you've created, in one place.</p>
        </div>
      ...
      <div className="panel table-panel">
        <div className="toolbar">
          <div className="search-box">
            ...
            <input
              ...
              placeholder="Search tracking number or address"
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <span>Try another search or create a shipment.</span>
          </div>
        ) : (
